...
export default class Projects {
  ...
  async all(teamName: string, options: { page?: number; limit?: number } = {}) {
    ...
    return (await this.client.request(
      `/teams/${teamName}/projects${params.size > 0 ? "?" + params.toString() : ""}`
   ...
   * @returns
   */
  async recently(teamName: string, projectName: string) {
    return (await this.client.request(`/teams/${teamName}/projects/${projectName}/recently`)) as APIResponse<Project[]>;
  }
  ...
  async update(
    ...
    return await this.client.request(`/teams/${teamName}/projects/${projectName}`, {
      method: "PATCH",
