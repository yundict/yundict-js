...
export async function cleanupTestResources(yundict: Yundict, teamName: string) {
	try {
		...
		console.warn(
			`Failed to cleanup test resources for team ${teamName}:`,
			error,
		);
		return false;
	}
...
export async function setupTestResources(
	...
	try {
		...
		const existingTeam = teamsRes.data?.find(
			(team) => team.name === config.TEAM_NAME,
		);
		if (!existingTeam) {
			throw new Error(
				`Test team "${config.TEAM_NAME}" not found. Please ensure the test team exists.`,
			);
		}
		...
		const existingProject = projectsRes.data?.find(
			(proj) => proj.name === config.PROJECT_NAME,
		);
		if (!existingProject) {
			throw new Error(
				`Test project "${config.PROJECT_NAME}" not found in team "${config.TEAM_NAME}". Please ensure the test project exists.`,
			);
		}
